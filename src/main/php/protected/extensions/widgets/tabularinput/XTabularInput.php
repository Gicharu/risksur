...
class XTabularInput extends CWidget {
	...
	public function run() {
		...
		if ($this->header) echo CHtml::tag($this->headerTagName, $this->headerHtmlOptions, $this->header);
	...
	$("#{$this->id} .{$this->addCssClass}").click(function(event){
		...
		$.ajax({
			success: function(html){
				//console.log('{$openInputTag}'+html+'{$this->getRemoveLinkAndIndexInput("'+index+'") }{$closeInputTag}');
...
SCRIPT;
	...
	protected function renderContent() {
		foreach ($this->models as $index => $model) {
			...
			$newForm = new CForm($this->elements, $model);
			$this->controller->renderPartial($this->inputView, array(
				'model' => $model,
				'index' => $index,
				'form' => $newForm
			));
	...
	protected function getAddLink() {
		...
		if ($this->addTemplate) $addLink = strtr($this->addTemplate, array(
			'{link}' => $addLink
		));
		return $addLink;
	}
	...
	protected function getRemoveLinkAndIndexInput($index) {
		...
		if ($this->removeTemplate) $removeLink = strtr($this->removeTemplate, array(
			'{link}' => $removeLink
		));
		return $removeLink;
	}
