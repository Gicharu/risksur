...
class EvaAttributes extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, description, attributeType', 'required'),
			array('name', 'length', 'max'=>50),
			array('attributeType', 'length', 'max'=>11),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('attributeId, name, description, attributeType', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('attributeType',$this->attributeType,true);
